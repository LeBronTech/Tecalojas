...
const WhatsAppButton = () => {
    ...
    useEffect(() => {
        ...
        const collapseTimer = setTimeout(() => setIsExpanded(false), 4500); // 3 seconds expanded
        return () => {
            clearTimeout(timer);
    ...
    return (
        <a 
            ...
            className={`fixed bottom-28 right-6 z-[60] flex items-center bg-[#25D366] text-white rounded-full shadow-2xl transition-all duration-700 ease-in-out h-14 ${isExpanded ? 'px-5 py-3 w-auto' : 'w-14 p-3 justify-center'}`}
        >
