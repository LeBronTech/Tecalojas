                ...
                <div className="p-4 border-t border-gray-200 dark:border-white/10 space-y-3 bg-opacity-50">
                    <div className="grid grid-cols-2 gap-2">
                        <a 
                            ...
                            onClick={onClose}
                            className={`flex items-center justify-center gap-2 px-3 py-3 rounded-xl transition-colors ${isDark ? 'bg-green-500/20 text-green-300 hover:bg-green-500/30' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}
                        >
                            ...
                            <span className="font-semibold text-xs whitespace-nowrap">WhatsApp</span>
                        </a>

                        <button 
                            onClick={handleShareCatalog}
                            className={`flex items-center justify-center gap-2 px-3 py-3 rounded-xl transition-colors ${isDark ? 'bg-fuchsia-500/20 text-fuchsia-300 hover:bg-fuchsia-500/30' : 'bg-fuchsia-100 text-fuchsia-700 hover:bg-fuchsia-200'} relative`}
                        >
                            <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12s-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.367 2.684 3 3 0 00-5.367-2.684z" /></svg>
                            <span className="font-semibold text-xs whitespace-nowrap">Compartilhar</span>
                            {shareTooltip && (
                                <div className={`absolute bottom-full right-1/2 translate-x-1/2 mb-2 ${isDark ? 'bg-white text-black' : 'bg-black text-white'} text-[10px] px-2 py-1 rounded whitespace-nowrap animate-bounce font-bold z-50`}>Link copiado!</div>
                            )}
                        </button>
                    </div>
                    ...
                    <div className={isAdmin ? "grid grid-cols-2 gap-2" : "w-full"}>
                        <button 
                            onClick={toggleTheme}
                            className={`flex items-center justify-between px-3 py-3 rounded-xl transition-colors ${isDark ? 'bg-black/20 text-gray-300 hover:bg-black/30' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'} w-full`}
                        >
                            <span className="text-xs font-semibold">{isDark ? 'Tema Escuro' : 'Tema Claro'}</span>
                            <div className={`w-10 h-5 rounded-full relative transition-colors ${isDark ? 'bg-fuchsia-600' : 'bg-gray-300'} flex-shrink-0 ml-1`}>
                        ...
                        {isAdmin && (
                            <button 
                                onClick={onLogout}
                                className="flex items-center justify-center gap-2 px-3 py-3 rounded-xl text-red-500 hover:bg-red-500/10 transition-colors w-full"
                            >
