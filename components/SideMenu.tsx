...
const SideMenu: React.FC<SideMenuProps> = ({ isOpen, onClose, onNavigate, onLogout, isAdmin, activeView }) => {
    ...
    const menuItems = [
        ...
        { label: 'Gera Chaves', view: View.GENERATE_KEYS, icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /> },
    ];
            ...
            <div className={`relative w-72 h-full shadow-2xl flex flex-col transition-transform animate-slide-in-left ${isDark ? 'bg-[#1A1129]' : 'bg-white'}`}>
                ...
                <div className="p-4 border-t border-gray-200 dark:border-white/10 space-y-3 bg-opacity-50">
                    <button 
                        ...
                        className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-colors ${isDark ? 'bg-blue-500/20 text-blue-300 hover:bg-blue-500/30' : 'bg-blue-100 text-blue-700 hover:bg-blue-200'}`}
                    >
                        <svg className="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /></svg>
                        <span className="font-semibold text-sm">Gerar Chave Gemini</span>
                    </button>
                    ...
                    {window.aistudio && (
                        <button 
                            ...
                            className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-colors ${isDark ? 'bg-fuchsia-500/20 text-fuchsia-300 hover:bg-fuchsia-500/30' : 'bg-fuchsia-100 text-fuchsia-700 hover:bg-fuchsia-200'}`}
                        >
                            <svg className="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                            <span className="font-semibold text-sm">Inserir Chave Gemini</span>
                        </button>
                    )}
