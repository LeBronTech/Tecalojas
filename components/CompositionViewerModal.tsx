...
const CompositionViewerModal: React.FC<CompositionViewerModalProps> = ({ compositions, startIndex, onClose, onViewProduct, onSaveComposition }) => {
    ...
    const handleGenerateEnvironment = async () => {
        ...
        try {
            ...
            // FIX: Always use process.env.API_KEY for initialization as per coding guidelines.
            ...
            if (e.message && e.message.includes('429')) {
                ...
                setError(`API ocupada. Tente em ${retryMatch ? Math.ceil(parseFloat(retryMatch[1])) : 'alguns'} segundos.`);
            } else {
                setError(e.message || "Falha na IA. Tente novamente.");
            }
        } finally {
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xl flex items-center justify-center z-[100] p-4 animate-fade-in" onClick={onClose}>
            ...
            <div className={`relative w-full max-w-xl max-h-full rounded-2xl border ${modalBgClasses}`} onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 overflow-y-auto max-h-[90vh]">
                     ...
                     <p className={`text-sm mb-4 text-center ${textClasses}`}>Composição de {currentComposition.size} almofadas.</p>
                        ...
                        <div className="flex-grow flex items-center justify-center">
                            <div className="flex flex-wrap justify-center items-start gap-2">
                                {currentComposition.products.map((p) => (
                                    <button key={p.id} onClick={() => onViewProduct(p)} className="flex flex-col items-center w-24 group">
                                        <img src={p.baseImageUrl} alt={p.name} className="w-24 h-24 rounded-lg shadow-lg object-cover" />
                    ...
                    <div className={`w-full aspect-square rounded-xl border p-4 flex flex-col items-center justify-center ${isDark ? 'bg-black/20 border-white/10' : 'bg-gray-100 border-gray-200'}`}>
                         <div className="flex-grow w-full flex items-center justify-center">
                             {currentComposition.imageUrl ? (
                                <img src={currentComposition.imageUrl} alt="IA" className="max-w-full max-h-full object-contain rounded-lg" />
                            ) : (
                                <p className={textClasses}>{isGenerating ? 'Gerando...' : 'Sem imagem gerada por IA.'}</p>
                            )}
                        </div>
                         <button onClick={handleGenerateEnvironment} disabled={isGenerating} className={`mt-4 font-bold py-2 px-4 rounded-lg text-sm transition-colors ${isDark ? 'bg-cyan-500/20 text-cyan-300 hover:bg-cyan-500/40' : 'bg-cyan-100 text-cyan-700 hover:bg-cyan-200'}`}>
