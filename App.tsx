...
const SideMenu: React.FC<SideMenuProps> = ({ isOpen, onClose, onLogout, onLoginClick, onPixClick, activeView, onNavigate, isLoggedIn, isAdmin, hasItemsToRestock, hasNewSaleRequests }) => {
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 left-0 h-full w-72 ${menuBgColor} shadow-2xl z-50 transform transition-transform duration-300 ease-in-out border-r ${borderColor} ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className={`p-6 ${textColor} flex flex-col h-full`}>
          ...
          <div className="mt-auto">
            ...
            <button onClick={toggleTheme} className={`w-full flex items-center p-3 rounded-lg text-left transition-colors ${itemBgHover}`}>
              <span className="mr-3">
                {theme === 'dark' 
              ...
              <span className="font-semibold">Mudar para Tema {theme === 'dark' ? 'Claro' : 'Escuro'}</span>
            </button>
            ...
            {isLoggedIn ? (
                 <button onClick={onLogout} className={`w-full flex items-center p-3 rounded-lg text-left transition-colors ${itemBgHover}`}>
                  <span className="mr-3">
                 ...
                 <button onClick={onLoginClick} className={`w-full flex items-center p-3 rounded-lg text-left transition-colors ${itemBgHover}`}>
                  <span className="mr-3">
                     <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
